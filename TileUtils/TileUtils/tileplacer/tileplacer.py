...
1. An EnvironmentSetter() / Placer() object pair is created based on the specified environment and
translation type.

2. The EnvironmentsSetter() object's setup() method is called.

3. The Placer() object's setup() method is called with the return value from the call to the
EnvironmentsSetter() object's setup() method as a parameter.

4. The Placer() object is configured.

5. For each tile in the tilemap that has a translation the Placer() object's placeTile() method is
called with the tile's layer number, x position, y position, and translation data as parameters. 

6. For each tile in the tilemap that has a translation the Placer() object's placeData() method is
called with the tile's layer number, x position, y position, and translation data as parameters. 

7. The Placer() object's teardown() method is called.

8. If it has been specified that output should be saved then the EnvironmentSetter() object's save()
method is called with the return value from the call to the Placer() object's teardown() method as a
parameter.
...
if __name__ == "__main__":
	...
	# This is a mess and should be fixed. There are too many overlapping exceptions.
	try:
